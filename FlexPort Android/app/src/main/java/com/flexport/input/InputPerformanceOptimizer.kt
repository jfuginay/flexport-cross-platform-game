...
class InputPerformanceOptimizer {
    ...
    private val vector2Pool = ObjectPool<Vector2>(
        factory = { Vector2() },
        reset = { it.set(0f, 0f) },
        initialSize = 50
    )
    private val touchEventPool = ObjectPool<TouchEvent>(
        factory = { TouchEvent(0, TouchEvent.TouchAction.DOWN, Vector2(), Vector2()) },
        reset = { /* TouchEvent is immutable, no reset needed */ },
        initialSize = 30
    )
    private val gestureEventPool = ObjectPool<GestureEvent>(
        factory = { GestureEvent.Tap(Vector2(), Vector2()) },
        reset = { /* GestureEvent is immutable, no reset needed */ },
        initialSize = 20
    )
    ...
    fun isSignificantMovement(pointerId: Int, worldX: Float, worldY: Float): Boolean {
        ...
        val distance = Vector2.dst(lastPosition.x, lastPosition.y, worldX, worldY)
    ...
    fun getPerformanceStats(): PerformanceStats {
        return PerformanceStats(
            ...
            vector2PoolSize = vector2Pool.size(),
            touchEventPoolSize = touchEventPool.size()
        )
    }
