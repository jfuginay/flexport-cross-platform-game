...
class SelectionManager(
    ...
    sealed class SelectionEvent {
        data class EntitySelected(val entityId: Int) : SelectionEvent()
        data class EntityDeselected(val entityId: Int) : SelectionEvent()
        data class SelectionCleared(val previousSelection: Set<Int>) : SelectionEvent()
        ...
        data class BoxSelectionCompleted(val selectedEntities: Set<Int>, val bounds: Rectangle) : SelectionEvent()
    }
    ...
    private fun findSelectableEntityAtPosition(x: Float, y: Float): com.flexport.ecs.core.Entity? {
        val entities = entityManager.getEntitiesWithComponent<SelectableComponent>()
        
        // Sort by priority if touchable
        val sortedEntities = entities.sortedByDescending { entity ->
    ...
    private fun findSelectableEntitiesInBounds(bounds: Rectangle): List<com.flexport.ecs.core.Entity> {
        val entities = entityManager.getEntitiesWithComponent<SelectableComponent>()
        val entitiesInBounds = mutableListOf<com.flexport.ecs.core.Entity>()
    ...
    private fun selectSingleEntity(entityId: Int) {
    ...
    private fun toggleEntitySelection(entityId: Int) {
    ...
    private fun selectEntity(entityId: Int, emitEvent: Boolean = true) {
    ...
    private fun deselectEntity(entityId: Int, emitEvent: Boolean = true) {
        if (entityId !in selectedEntities) return
     ...
     */
    fun isEntitySelected(entityId: Int): Boolean = entityId in selectedEntities
     ...
     */
    fun selectEntities(entityIds: Collection<Int>, clearPrevious: Boolean = true) {
        if (clearPrevious) {
            clearSelection(emitEvent = false)
        }
